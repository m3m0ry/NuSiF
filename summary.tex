...
With Figures~\ref{fig:deriv-def} and~\ref{fig:deriv-def2} all necesarry discretization of derivateves is provided. However in some case this discretization results into unphysical oscilations.
...
With central differences, oscillations will start to occur when $\delta x > \frac{2}{\abs{k}}$ $\rightarrow$ instable discretization.
...
Alternativly: "donor-cell-scheme" for the discretization of $\frac{d(ku)}{dx}$:
\begin{figure}[h]
	\centering
...
Discretization in 2D: initially $\Omega = [0,a] \times [0,b] \subset \mathds{R}^2$
...
\end{matrix}\right.	 \]
	...
	\caption{Linear system of eqations}
	\label{fig:sys-of-eq}
\end{figure}


Bondary conditions can be incorporated by setting $u_{i,j} = g(x_{i,j})$ for $i \in \lbrace 0, i_{max} \rbrace$ or $j \in \lbrace 0,j_{max} \rbrace$. If the PDE contains 1st-order derivatives, it may be necessary to discretize them by techniques introduced in 1D using upwind or donor-cell discretization.
...
\begin{itemize}
	...
	\item vertifal velocity $v$ in midpoints of horizontal cell edges
\end{itemize}
%TODO picture
...
Staggered grid $\Rightarrow$ no pressure oscilations\\
...
Discretization on a staggered grid: Continuity equation TODO2.20 will be discretized at center of each cell $i = 1, \dots, i_{max}; j = 1, \dots, j_{max}$ by replacing $\frac{\delta u}{\delta x}$ and $\frac{\delta v}{\delta y}$ by central differences
