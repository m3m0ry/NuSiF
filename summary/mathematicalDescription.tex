...
\begin{itemize}
	...
	\cdot \begin{pmatrix}
		...
		\frac{\delta}{\delta y}
	\end{pmatrix}\right)
	= (u \frac{\delta}{\delta x} + v \frac{\delta}{\delta y})$
\item $(\vec{u} \text{ grad} ) \vec{u} = \left( u \frac{\delta}{\delta x} + v \frac{\delta}{\delta y}\right)
   \begin{pmatrix}
		u \\
		v
	...
	\item From now on in 2D: $\vec{x} = \begin{pmatrix}
	x\\
	y
	\end{pmatrix}; \vec{u} = \begin{pmatrix}
	u\\
	v
	\end{pmatrix}; \vec{g} = \begin{pmatrix}
	g_x \\
	g_y
	\end{pmatrix}$
\end{itemize}
...
To formulate the boundary condition, we introduce $\varphi_n$ velocity component normal to the boundary and $\varphi_t$ velocity component tangential to the boundary. When the boundary is aligned with the coordinate direction: 
...
\varphi_n = v\\
\varphi_t = u
\end{matrix} \right\rbrace$ horizontal boundary $\left\lbrace \frac{\delta \varphi_n}{\delta n} = \frac{\delta v}{\delta y},\frac{\delta \varphi_t}{\delta n} = \frac{\delta u}{\delta y} \right\rbrace$
\end{itemize}
